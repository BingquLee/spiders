...
def read_timest():
    ...
    date = \
    con.xpath('//div[@class="stream"]/ol[@id="stream-items-id"]/li[last()]//div[@class="content"]//small/a/@title')[0].decode('utf-8').split(" - ")[1]
...
def crawl_twitter(url, start_strdate, end_strdate):
    ...
    for li in lis:
        ...
        try:
            # 回复数
            retweet_count = int(li.xpath(
                './/div[@class="content"]/div[4]/div[2]/div[1]/button/span[@class="ProfileTweet-actionCount "]/span/text()')[
                                    0])
        except:
            retweet_count = 0
        try:
        ...
        if created_at >= start_timest and created_at <= end_timest:
            ...
            item['keyword'] = 'Drew Gray Miller'  # 关键词
            item['url'] = url  # 昵称
            ...
            with open('Twitter_Pennsylvania.json', 'a') as f:
                f.write(json.dumps(item, ensure_ascii=False) + '\n')


if __name__ == '__main__':
    ConorLamb_url = "https://twitter.com/conorlambpa"
    DrewGrayMiller_url = "https://twitter.com/DrewGrayMiller"
    # crawl_twitter('Mostafa Hashemitaba')
    # crawl_twitter('Ebrahim Raisi')
    # crawl_twitter('Hassan Rouhani')
    # crawl_twitter('Mostafa Mir-Salim')
    # crawl_twitter(ConorLamb_url, '2017-10-01', '2018-03-13')
    crawl_twitter(DrewGrayMiller_url, '2017-10-01', '2018-03-13')
